...
public class EjemploRegex {
    ...
    private static String[] testNombres = {
        "prueba.txt", "prueba2.jpg", "otro.dat", "arantza.doc", "mikel.txt", "dir", "prueba3."
    };
    ...
    public static void main(String[] args) {
        ...
        for (String s : testNombres) {
            if (pattern.matcher(s).matches()) {  // Se comprueba si pasa el filtro
                System.out.println(String.format("+ Si cumple: %s", s));
            } else { 
            	System.out.println(String.format("- No cumple: %s", s));
            }
        }
    }
}